...
class TiposControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create tipo" do
    assert_difference('Tipo.count') do
      post tipos_url, params: { tipo: { nome: @tipo.nome, status: @tipo.status, valor_tipo: @tipo.valor_tipo } }
    end
  ...
  test "should update tipo" do
    patch tipo_url(@tipo), params: { tipo: { nome: @tipo.nome, status: @tipo.status, valor_tipo: @tipo.valor_tipo } }
