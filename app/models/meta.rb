class Meta < ApplicationRecord
  ...
  scope :periodo , -> (data_referencia) { where("to_char(datameta, 'MM') = ? and to_char(datameta, 'YYYY') = ?", data_referencia.to_date.strftime('%m'), data_referencia.to_date.strftime('%Y')) }
  ...
  scope :metas_not_done, -> (datareferencia) { where("to_char(datameta, 'MM') = ? AND to_char(datameta, 'YYYY') = ? AND status = ?", datareferencia.to_date.strftime('%m'), datareferencia.to_date.strftime('%Y'), false) }
  scope :meta_done, -> (datareferencia) { where("to_char(datameta, 'MM') = ? AND to_char(datameta, 'YYYY') = ? AND status = ?", datareferencia.to_date.strftime('%m'), datareferencia.to_date.strftime('%Y'), true) }
